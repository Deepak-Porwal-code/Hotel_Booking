...
const RadioButton = ({label , selected = false , onchange = ()=>{}})=>{
    return (
        <label className="flex gap-3 items-center cursor-pointer mt-2 text-sm">
            <input type="radio" namer="sortOption" checked={selected} onChange={(e)=>onchange (label)} />
            <span>{label}</span>
        </label>
    )
}
    ...
    <div className="flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24">
      <div>
        ...
        {roomsDummyData.map((room) => (
          <div
            key={room._id}
            className="flex flex-col md:flex-row items-center py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0 "
          >
            <img
              onClick={() => {
            ...
            <div className="md:w-1/2 flex flex-col gap-2">
              ...
              <div className="flex flex-wrap items-center mt-3 mb-6 gap-4">
                {room.amenities.map((item, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-2 px-3 py-2 rounded-l-lg bg-[#F5F5FF]/70"
                  >
                    <img
